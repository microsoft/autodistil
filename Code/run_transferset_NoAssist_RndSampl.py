...
from transformers import glue_convert_examples_to_features_transferset as convert_examples_to_features_transferset
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    #   "guid": "train-2001",
    #   "label": "1",
    #   "text_a": "Law enforcement sources who spoke on condition of anonymity confirmed to The Associated Press that Limbaugh was being investigated by the Palm Beach County state attorney 's office .",
    #   "text_b": "Law enforcement officials confirmed that Limbaugh was being investigated by the Palm Beach County , Fla . , state attorney 's office ."
    # }
...
#set random seeds
GLOBAL_SEED = 123 # int(os.getenv("PYTHONHASHSEED"))
...
def generate_sequence_data(args, MAX_SEQUENCE_LENGTH, input_file, tokenizer, label_list=None, unlabeled=False, special_tokens=None, do_pairwise=False, do_NER=False):
    ...
    with tf.io.gfile.GFile(input_file, "r") as f:
      ...
      for line in reader:
        n_line += 1
        if n_line % 100000 == 0:
            print('n_line (Subho code): ', n_line)
        # if n_line == 100:
        #     break
    ...
    features = convert_examples_to_features_transferset(X1, X2,
                                            tokenizer,
                                            label_list=label_list,
                                            max_length=args.max_seq_length,
                                            pad_on_left=bool(args.model_type in ['xlnet']),                 # pad on the left for xlnet
    ...
    # Convert to Tensors and build dataset
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long) # <class 'torch.Tensor'>, [3668, 128]
    all_attention_mask = torch.tensor([f.attention_mask for f in features], dtype=torch.long) # <class 'torch.Tensor'>, [3668, 128]
    all_token_type_ids = torch.tensor([f.token_type_ids for f in features], dtype=torch.long) # <class 'torch.Tensor'>, [3668, 128]
    ...
    # if output_mode == "classification":
    #     all_labels = torch.tensor([f.label for f in features], dtype=torch.long) # <class 'torch.Tensor'>, [3668]
    # elif output_mode == "regression":
    #     all_labels = torch.tensor([f.label for f in features], dtype=torch.float)
    
    all_labels = torch.tensor(np.array(y), dtype=torch.long)
...
def main():
    ...
    parser.add_argument("--transfer_file", default=None, type=str, required=True,
                        help="transfer data for distillation")
    ...
    if args.do_train:
        ...
        train_dataset = generate_sequence_data(args, args.max_seq_length, args.transfer_file, tokenizer, label_list=None, unlabeled=True, special_tokens=None, do_pairwise=True, do_NER=False)
...
if __name__ == "__main__":
    ...
    #     '--transfer_file', '/home/t-dongkuanxu/Pretrained-Language-Model/Data_and_Models/TransferSet_Subho/para-nmt-5m-processed.txt',
