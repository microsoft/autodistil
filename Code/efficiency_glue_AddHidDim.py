...
from transformers import (BertConfig,
                                  BertForSequenceClassification, BertForSequenceClassification_v1, BertTokenizer,
                                  RobertaConfig,
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task in eval_task_names:
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, evaluate=True) # it is training set now!!!

        eval_output_dir = os.path.join(args.output_dir,
                                       args.model_type + '_' + args.hidden_mult + '_' + args.width_mult + '_' + args.depth_mult + '_eval')
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    examples = processor.get_train_examples(args.data_dir) if evaluate else processor.get_train_examples(args.data_dir)
...
def main():
    ...
    parser.add_argument('--width_mult', type=str, default='1.',
                        help="the possible widths used for training, e.g., '1.' is for default")
    parser.add_argument('--hidden_mult', type=str, default='1.',
                        help="the possible hiddens used for training, e.g., '1.' is for default")
    ...
    model.apply(lambda m: setattr(m, 'hidden_mult', float(args.hidden_mult)))
    ...
    results, start = evaluate(args, model, tokenizer)
    end = time.time()
