                    ...
                    for width_mult in sorted(args.width_mult_list, reverse=True):
                        ...
                        for hidden_mult in sorted(args.hidden_mult_list, reverse=True):
                            ...
                            for intermediate_mult in sorted(args.intermediate_mult_list, reverse=True):
                                ...
                                # logger.info("results_TrainEval: %s ", results_TrainEval)
                                ...
                                acc_TrainEval.append(list(results_TrainEval.values())[0])
                                if args.task_name == "mnli":
...
def evaluate_TrainEval(args, model, tokenizer, TrainEval_dataset, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        preds = None
        out_label_ids = None
        for batch in tqdm(eval_dataloader, desc="Evaluating_on_TrainEval"):
            model.eval()
            batch = tuple(t.to(args.device) for t in batch)
        ...
        output_eval_file = os.path.join(eval_output_dir, "eval_TrainEval_results.txt") # wirte all the results to the same file
        with open(output_eval_file, "a") as writer:
            logger.info("***** Eval results {} *****".format(prefix))
            for key in sorted(result.keys()):
                logger.info("TrainEval  %s = %s", key, str(result[key]))
                # writer.write("TrainEval %s = %s\n" % (key, str(result[key])))
            # writer.write("\n")
    return results


def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_from_train=False):
    ...
    # according to https://discuss.pytorch.org/t/torch-utils-data-dataset-random-split/32209/4
    # split extra val set from train for val score
    if not evaluate and eval_from_train:
        lengths = [int(len(dataset)*(1 - args.ratio_split)), int(len(dataset)*args.ratio_split)]
        # subsetA, subsetB = random_split(dataset, lengths, generator=torch.Generator().manual_seed(42)) # pytorch 1.4.0 does not support "generator"; 1.6.0 supports it
        subsetA, subsetB = random_split(dataset, lengths) # fixed
...
def main():
    ...
    parser.add_argument("--ratio_split", default=0.2, type=float,
                        help="ratio to split val from train.")
