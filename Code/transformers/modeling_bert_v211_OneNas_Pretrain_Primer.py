...
class BertSelfAttention_v1(nn.Module):
    def __init__(self, config):
        ...
        # added by DK, from Primer (https://arxiv.org/abs/2109.08668)
        self.kernel_size = 3
        # Depthwise conv: groups == in_channels
        # need to revise 'Conv1d' for dynamic hid_size !!
        self.dconv_q = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=self.kernel_size, groups=config.hidden_size)
        self.dconv_k = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=self.kernel_size, groups=config.hidden_size)
        self.dconv_v = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=self.kernel_size, groups=config.hidden_size)
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        # from Primer (https://arxiv.org/abs/2109.08668)
        mixed_query_layer = mixed_query_layer.permute(0, 2, 1) # [batch, 'hid', length]
        mixed_key_layer = mixed_key_layer.permute(0, 2, 1)
        mixed_value_layer = mixed_value_layer.permute(0, 2, 1)

        # from Primer 
        # padding, equally for left and right sides
        mixed_query_layer = F.pad(mixed_query_layer, (int((self.kernel_size-1)/2.), (self.kernel_size-1) - int((self.kernel_size-1)/2.)), 'constant', 0)
        mixed_key_layer = F.pad(mixed_key_layer, (int((self.kernel_size-1)/2.), (self.kernel_size-1) - int((self.kernel_size-1)/2.)), 'constant', 0)
        mixed_value_layer = F.pad(mixed_value_layer, (int((self.kernel_size-1)/2.), (self.kernel_size-1) - int((self.kernel_size-1)/2.)), 'constant', 0)

        # from Primer 
        mixed_query_layer = self.dconv_q(mixed_query_layer)
        mixed_key_layer = self.dconv_k(mixed_key_layer)
        mixed_value_layer = self.dconv_v(mixed_value_layer)

        # from Primer 
        mixed_query_layer = mixed_query_layer.permute(0, 2, 1) # [batch, length, hid]
        mixed_key_layer = mixed_key_layer.permute(0, 2, 1)
        mixed_value_layer = mixed_value_layer.permute(0, 2, 1)
...
class BertLayer_v1(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        # added by DK, from Primer (https://arxiv.org/abs/2109.08668)
        intermediate_output = intermediate_output ** 2
...
class BertForPreTraining_v1_Primer(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForPreTraining_v1_Primer, self).__init__(config)
