            ...
            for depth_mult in sorted(args.depth_mult_list, reverse=True):
                ...
                for width_mult in sorted(args.width_mult_list, reverse=True):
                    ...
                    # stage 2: width- and depth- adaptive
                        ...
                        student_att, teacher_att = student_atts[-1], atts_max_all[width_idx][-1]
                        student_att = torch.where(student_att <= -1e2, torch.zeros_like(student_att).to(args.device),
                                        student_att)
                        teacher_att = torch.where(teacher_att <= -1e2, torch.zeros_like(teacher_att).to(args.device),
                                        teacher_att)
...
if __name__ == "__main__":
    ...
    #     '--task_name', 'MNLI',
    #     '--do_train',
    #     '--data_dir', '/home/jiachao/data/DongkuanXu/transformers/glue_data/MNLI/',
    #     '--model_dir', '/home/jiachao/data/DongkuanXu/TinyBERT/finetuned_BERTs/bert_base_uncased_mnli',
    #     '--output_dir', '/home/jiachao/data/DongkuanXu/DynaBERT_DK/Pretrained-Language-Model/DynaBERT/outputs/glue/MNLI/DYNABERTW_debug',
    #     '--max_seq_length', '128',
    #     '--learning_rate', '2e-5',
