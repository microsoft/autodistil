...
from transformers import (BertConfig, BertForSequenceClassification, BertForSequenceClassification_v1, BertForSequenceClassification_v1_AddHidLoss, BertForSequenceClassification_v1_AddHidLoss_ForPreTraining, BertTokenizer,
                                  RobertaConfig,
...
MODEL_CLASSES = {
    ...
    'bert': (BertConfig, BertForSequenceClassification_v1_AddHidLoss_ForPreTraining, BertTokenizer),
    'roberta': (RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer)
}


def set_seed(args):
    random.seed(args.seed) # it is commented in AutoFormer, but not in AutoTinyBERT
...
def train(args, model, tokenizer, teacher_model=None, samples_per_epoch=None, num_data_epochs=None):
    ...
    for epoch_wholeset in trange(int(args.num_train_epochs_wholeset), desc="Local_rank {}, Epoch_wholeset".format(args.local_rank)):
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Local_rank {}, Epoch".format(args.local_rank)):
            ...
            # epoch_dataset = PregeneratedDataset(epoch=epoch, training_path=args.pregenerated_data, tokenizer=tokenizer, num_data_epochs=num_data_epochs, reduce_memory=args.reduce_memory, output_cache_dir=args.output_cache_dir, args=args)
            
            data_file_list = files[int(epoch*step_files) : int((epoch+1)*step_files)]
            #data_file_list = files[:2]
            epoch_dataset = pretraining_dataset(data_file_list, args.max_predictions_per_seq)
            ...
            with tqdm(total=len(train_dataloader), desc="Local_rank {}, Epoch {}".format(args.local_rank, epoch)) as pbar:
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Local_rank {}, Iteration".format(args.local_rank), ascii=True)):
                    ...
                    # for wiki_book data on KD + pretrain
                    # batch[3][:, 0]: 'labels' is meaningless when KD; [:,0] for matching shape
                    inputs = {'input_ids': batch[0], 'attention_mask': batch[2], 'labels': None, 'masked_lm_labels': batch[3], 'token_type_ids': batch[1] if args.model_type in ['bert'] else None, 'next_sentence_label': batch[4]}
                    inputs_stu = {'input_ids': batch[0], 'attention_mask': batch[2], 'labels': None, 'masked_lm_labels': batch[3], 'token_type_ids': batch[1] if args.model_type in ['bert'] else None, 'is_student': True, 'next_sentence_label': batch[4]}
                    ...
                    for idx_sub in range(len(subs_sampled)):
                        ...
                        if args.training_phase == 'dynabert':
                            if getattr(args, 'data_aug'):
                                print("Doing data_aug")

                            else: 
                                # hard_loss, student_logit, student_reps, student_atts, _ = model(**inputs_stu)
                                pretrain_loss, student_logit, student_reps, student_atts, student_atts_QQ, student_atts_KK, student_atts_VV, _ = model(**inputs_stu)
                                ...
                                #                 teacher_att)
                                # att_loss_teacher_att = loss_mse(torch.sum(student_att, dim=1), torch.sum(teacher_att.detach(), dim=1))


                                student_att_QQ, teacher_att_QQ = student_atts_QQ[-1], teacher_atts_QQ[-1]

                                # print('')
                                # print('student_att_QQ[0,0,:,:]: ', student_att_QQ[0,0,:,:])
                                # print('')
                                ...
                                # print('')
                                # print('After student_att_QQ[0,0,:,:]: ', student_att_QQ[0,0,:,:])
                                # print('')
                                ...
                                loss = args.depth_lambda1 * logit_loss + args.depth_lambda3 * att_loss + args.depth_lambda2 * rep_loss + args.pretrain_lambda * pretrain_loss
                                ...
                                if (global_step + 1) % args.printloss_step == 0 and args.local_rank in [-1, 0]:
                                    ...
                                    print("pretrain_loss", pretrain_loss.item())
...
def main():
    ...
    parser.add_argument("--num_train_epochs", default=3.0, type=float,
                        help="The number of parts that wiki+book is divided into.")
    ...
    parser.add_argument("--pretrain_lambda", default=1.0, type=float,
                        help="pretraining loss coef. of student")
    ...
    if args.do_train:
        ...
        if teacher_model:
            ...
            train(args, model, tokenizer, teacher_model)
        else:
